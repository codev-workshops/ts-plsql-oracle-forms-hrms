# Component Mapping: Oracle Forms → Spring Boot + React on PostgreSQL
...
Every JPA entity maps to the **PostgreSQL** table migrated from the `HRMS` schema table named in [DATA_DICTIONARY.md](DATA_DICTIONARY.md) (lower-case identifiers, type mapping per [MODERNIZATION_BLUEPRINT.md](MODERNIZATION_BLUEPRINT.md) §10); IDs come from the PostgreSQL re-creations of the `SEQ_*` sequences (`schema/sequences/hrms_sequences.sql`) via `@SequenceGenerator(allocationSize = 1)`, restarted with an offset at each module's cutover so legacy (Oracle) and new (PostgreSQL) rows never collide during the parallel-run period ([CUTOVER_PLAN.md](CUTOVER_PLAN.md) §2 rule 4). **No target component calls a `PKG_*` procedure or attaches a `.pll`**: every "Source → Target" row below is a re-implementation in Java/React; the legacy package runs on Oracle only as the characterization oracle for that row's parallel-run diff ([TEST_STRATEGY.md](TEST_STRATEGY.md) §3).
...
Source: `forms/xml-exports/HRMS_PAYROLL.xml` (blocks `PAY_PERIOD`, `PAYROLL_RUN`; **0 LOVs**; header claims 4 blocks / 3 LOVs and tab "Pay Details" – `PAYROLL_DETAIL`, `PAYSLIP_SUMMARY` are not present), `plsql/packages/PKG_PAYROLL.pks/.pkb`. Strategy: **rewrite** – pure Java `PayrollRunService` / `TaxEngine`; no façade over `PKG_PAYROLL`, which is used only as the shadow-mode golden oracle on Oracle ([MODERNIZATION_BLUEPRINT.md](MODERNIZATION_BLUEPRINT.md) §4.3). Phase 4.
...
| `PKG_AUDIT.log_action(table, id, action, user)` | `audit/AuditService` + `AuditLog` (`AUDIT_LOG`); `CHK_AUDIT_ACTION` widened to include `STATUS_CHANGE` (ARCH-04); `REQUIRES_NEW` transaction replaces `PRAGMA AUTONOMOUS_TRANSACTION` (no PostgreSQL equivalent) | Called from every service write |
...
| `PKG_REPORTING` (ref cursors) | `reporting/ReportingController` + `ReportingService` – pure Java rewrite; read-only JPA/SQL queries on the PostgreSQL schema (recursive CTE for the org hierarchy), JSON/CSV endpoints (Phase 5). No JDBC bridge to the package. | Ref-cursor procedures characterised on Oracle as the row-for-row oracle, then dropped; the Oracle `VW_*` views are retained on the legacy side as reconciliation oracles ([TEST_STRATEGY.md](TEST_STRATEGY.md)) |
